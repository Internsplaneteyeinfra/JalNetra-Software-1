#!/usr/bin/env python3
"""
run_mula_mutha.py — JalNetra demo for Mula–Mutha river (Pune).

Runs the full pipeline on synthetic data shaped for the Pune stretch,
including villages along the river and the user KML patch (reach R06).

Usage:
    python run_mula_mutha.py                          # pipeline + serve on port 8001
    python run_mula_mutha.py --no-serve               # export JSON only
    python run_mula_mutha.py --refresh-minutes 30     # serve + re-export every 30 min

Output:
    dashboard/mula_mutha/dashboard.html
    dashboard/mula_mutha/dashboard_data.json
    dashboard/mula_mutha/river_patch.kml
"""
import argparse
import os
import shutil
...
cfg.REACHES_PROVIDER = cfg.mula_mutha_reaches
cfg.FORECAST_DAYS = 14

from jalnetra.config import (  # noqa: E402
    FORECAST_DAYS,
    MULA_MUTHA_KML_REACH,
...
DASH = os.path.join(HERE, "dashboard", "mula_mutha")
...
def run_pipeline(*, quiet=False):
    ...
    if not quiet:
        print("=" * 64)
        print("JalNetra — Mula–Mutha (Pune) · SYNTHETIC DEMO")
        print("=" * 64)

    src = DemoSource(seed=101, end=TODAY_END)
    ...
    rain_fc = rainfall_forecast(fdates, seed=108)
    ...
    lin = lineage("mula_mutha_export", dict(models={t: "gbm+conformal" for t in TARGETS},
    ...
    meta = dict(
        ...
        kml_reach_id=MULA_MUTHA_KML_REACH,
        history_days=HISTORY_DAYS,
        forecast_days=FORECAST_DAYS,
        demo_notice=(
            "MULA–MUTHA SYNTHETIC DEMO — Sangam–Bund Garden to Hadapsar–Manjari "
            "(7 reaches). KML patch = reach R06 (Bund Garden–Dhanori). "
...
def main(*, serve_after=True, refresh_minutes=0, port=8001):
    run_pipeline()
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Mula–Mutha JalNetra dashboard pipeline")
    ...
    ap.add_argument("--port", type=int, default=8001, help="HTTP port (default 8001)")
    args = ap.parse_args()
